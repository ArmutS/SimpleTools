<script lang="ts">
  ...
  // Settings
  let text: string = "CONFIDENTIAL";
  let rotation: number = 45;
    ...
    try {
      ...
      if (selected && !Array.isArray(selected)) {
        ...
        if (filename && !outputPath) {
          outputPath = `${directory}/watermarked_${filename}`;
        }
      }
    } catch (error) {
      status = `Hata: ${error}`;
  ...
  async function addWatermark() {
    ...
    if (!text.trim()) {
      status = "Filigran metni girmelisiniz!";
      return;
    }
    ...
    try {
      await invoke("pdf_watermark", {
        request: {
          ...
          rotation: rotation,
          opacity: opacity,
          font_size: 48, // Fixed for now
          color: "#000000", // Not fully implemented in backend yet
        },
      });
...
<div class="layout">
  ...
  <div class="panel left-panel">
    <div class="header">
      <h2>Watermark PDF</h2>
      <p>PDF sayfalarına filigran ekleyin</p>
    </div>
    ...
    <div class="controls-block">
      ...
      <div class="settings-row">
        <div class="setting-group">
          <label for="rotation">Açı ({rotation}°)</label>
          <input
            id="rotation"
            type="range"
            min="0"
        ...
        <div class="setting-group">
          <label for="opacity">Opaklık ({Math.round(opacity * 100)}%)</label>
          <input
            id="opacity"
            type="range"
            min="0.1"
      ...
      <div class="input-group">
        ...
        <div class="path-select">
          <input
            ...
            placeholder="/path/to/watermarked.pdf"
          />
    ...
    <div class="action-block">
      ...
      >
        ...
        {:else}
          <i class="nf-md-watermark"></i> ETİKETLE
        {/if}
      </button>
  ...
  <div class="panel right-panel">
    <div class="preview-placeholder">
      <i class="nf-md-watermark"></i>
      <h3>Watermark PDF</h3>
      <p>Belgelerinize telif veya gizlilik etiketi ekleyin.</p>
    </div>
  </div>
</div>
