<script lang="ts">
  ...
  // Settings
  let pageInputValue: string = ""; // "1,3,5" etc.
    ...
    try {
      ...
      if (selected && !Array.isArray(selected)) {
        ...
        if (filename && !outputPath) {
          outputPath = `${directory}/deleted_pages_${filename}`;
        }
      }
    } catch (error) {
      status = `Hata: ${error}`;
  ...
  async function deletePages() {
    ...
    if (!pageInputValue.trim()) {
      status = "Silinecek sayfaları belirtmelisiniz!";
      return;
    }
    ...
    try {
      ...
      // Backend expects u32, ensure valid numbers
      pages = pages.filter((p) => p > 0);

      if (pages.length === 0) {
        throw new Error("Geçerli sayfa numarası bulunamadı.");
      }
...
<div class="layout">
  ...
  <div class="panel left-panel">
    <div class="header">
      <h2>Delete Pages</h2>
      <p>PDF'ten istenmeyen sayfaları silin</p>
    </div>
    ...
    <div class="controls-block">
      ...
      <div class="input-group">
        <label for="pages" style="color: #f38ba8;">Silinecek Sayfalar:</label>
        <input
          id="pages"
          type="text"
          bind:value={pageInputValue}
          placeholder="Örn: 2,4,7-9"
        />
        <small style="color:var(--text-muted); font-size: 0.75rem;"
          >Bu sayfalar kalıcı olarak silinecektir.</small
        >
      </div>
    ...
    <div class="action-block">
      <button
        class="merge-btn delete-action-btn"
        on:click={deletePages}
        disabled={!selectedFile || !outputPath || isProcessing}
      >
        {#if isProcessing}
          <i class="nf-md-loading nf-spin"></i> İşleniyor...
        {:else}
          <i class="nf-md-delete"></i> SAYFALARI SİL
        {/if}
      </button>
      ...
      {#if status && !status.includes("Başarılı") && !status.includes("Hazır") && !status.includes("İşleniyor") && !status.includes("Siliniyor")}
  ...
  <div class="panel right-panel">
    <div class="preview-placeholder">
      <i class="nf-md-delete_forever"></i>
      <h3>Delete Pages</h3>
      <p>İstenmeyen sayfaları PDF dosyanızdan çıkarın.</p>
    </div>
  </div>
</div>
