    ...
    try {
      ...
      if (selected && !Array.isArray(selected)) {
        ...
        if (filename && !outputPath) {
          outputPath = `${directory}/compressed_${filename}`;
        }
      }
    } catch (error) {
      status = `Hata: ${error}`;
...
<div class="layout">
  ...
  <div class="panel left-panel">
    <div class="header">
      <h2>Compress PDF</h2>
      <p>PDF dosya boyutunu küçültün</p>
    </div>
    ...
    <div class="controls-block">
      ...
      <div class="settings-row">
        <div class="setting-group">
          <label for="quality">Sıkıştırma Seviyesi</label>
          <select id="quality" bind:value={quality}>
            <option value="low">Düşük (Daha iyi kalite)</option>
            <option value="medium">Orta (Dengeli)</option>
            <option value="high">Yüksek (En küçük boyut)</option>
          </select>
        </div>
      </div>
  ...
  <div class="panel right-panel">
    <div class="preview-placeholder">
      <i class="nf-md-compress"></i>
      <h3>Compress PDF</h3>
      <p>PDF dosyanızdaki gereksiz verileri temizleyerek boyutunu küçültür.</p>
    </div>
  </div>
</div>
