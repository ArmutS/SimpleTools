<script lang="ts">
    ...
    try {
      ...
      if (selected && !Array.isArray(selected)) {
        ...
        if (filename && !outputPath) {
          outputPath = `${directory}/protected_${filename}`;
        }
      }
    } catch (error) {
      status = `Hata: ${error}`;
  ...
  async function protectPdf() {
    ...
    if (!userPassword && !ownerPassword) {
      status = "En az bir şifre (Kullanıcı veya Sahip) girmelisiniz!";
      return;
    }
...
<div class="layout">
  ...
  <div class="panel left-panel">
    ...
    <div class="controls-block">
      <label>Güvenlik Ayarları</label>

      <div class="input-group">
        <label for="user-pass">Açılış Şifresi (Kullanıcı):</label>
        <input
          id="user-pass"
          type="password"
          bind:value={userPassword}
          placeholder="İsteğe bağlı - dosyayı açmak için"
        />
      </div>

      <div class="input-group">
        <label for="owner-pass">Düzenleme Şifresi (Sahip):</label>
        <input
          id="owner-pass"
          type="password"
          bind:value={ownerPassword}
          placeholder="Zorunlu değil ama önerilir"
        />
      </div>
      ...
      <div class="input-group">
        ...
        <div class="path-select">
          <input
            ...
            placeholder="/path/to/protected.pdf"
          />
  ...
  <div class="panel right-panel">
    <div class="preview-placeholder">
      <i class="nf-md-shield_lock"></i>
      <h3>Protect PDF</h3>
      <p>Belgelerinizi yetkisiz erişime karşı koruyun.</p>
    </div>
  </div>
</div>
