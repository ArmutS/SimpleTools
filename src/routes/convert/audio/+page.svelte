<script lang="ts">
  ...
  const TARGET_FORMATS = [
    ...
    { id: "ogg", label: "OGG Audio (.ogg)" },
    { id: "aac", label: "AAC Audio (.aac)" },
  ];
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "Audio Files", extensions: ACCEPTED_EXTENSIONS }],
      });
  ...
  <div class="panel main-panel">
    <div class="unified-card">
      <div class="header">
        ...
        <p>Convert MP3, WAV, FLAC, and other audio formats</p>
      </div>
