<script lang="ts">
  ...
  const ACCEPTED_EXTENSIONS = ["zip", "rar", "7z", "tar", "gz", "bz2", "xz"];
  const TARGET_FORMATS = [
    { id: "zip", label: "ZIP Archive (.zip)" },
    { id: "7z", label: "7Z Archive (.7z)" },
    { id: "tar", label: "TAR Archive (.tar)" },
    { id: "targz", label: "TAR.GZ (.tar.gz)" },
  ];
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "Archives", extensions: ACCEPTED_EXTENSIONS }],
      });
  ...
  <div class="panel main-panel">
    <div class="unified-card">
      <div class="header">
        ...
        <p>Convert RAR, 7Z, TAR, ZIP files</p>
      </div>
        ...
        <div class="file-list-scroll">
          {#if selectedFiles.length === 0}
            <div class="empty-state">
              <i class="nf-md-folder_zip"></i>
              <p>Drag archives here</p>
            </div>
          {:else}
