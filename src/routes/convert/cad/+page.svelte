<script lang="ts">
  ...
  const ACCEPTED_EXTENSIONS = ["stl", "obj", "step", "stp", "fbx", "gcode"];
  const TARGET_FORMATS = [
    { id: "stl", label: "STL Mesh (.stl)" },
    { id: "obj", label: "OBJ Model (.obj)" },
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "3D Models", extensions: ACCEPTED_EXTENSIONS }],
      });
  ...
  async function convert() {
    ...
    try {
      const result = await invoke("convert_cad", {
...
<div class="layout">
  <div class="panel main-panel">
    <div class="unified-card">
      <div class="header">
        <h2>3D/CAD Converter</h2>
        <p>Convert STL, OBJ, STEP, G-Code</p>
      </div>
        ...
        <div class="file-list-scroll">
          {#if selectedFiles.length === 0}
            <div class="empty-state">
              <i class="nf-md-cube_outline"></i>
              <p>Drag 3D models here</p>
            </div>
          {:else}
