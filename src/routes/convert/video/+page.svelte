<script lang="ts">
  ...
  const TARGET_FORMATS = [
    ...
    { id: "gif", label: "GIF Animation (.gif)" },
    { id: "mp3", label: "MP3 Audio (Extract Audio)" },
  ];
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "Video Files", extensions: ACCEPTED_EXTENSIONS }],
      });
  ...
  <div class="panel main-panel">
    <div class="unified-card">
      <div class="header">
        ...
        <p>Convert MP4, MKV, AVI, and other video formats</p>
      </div>
