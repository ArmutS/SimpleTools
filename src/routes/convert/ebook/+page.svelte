<script lang="ts">
  ...
  const TARGET_FORMATS = [
    ...
    { id: "mobi", label: "MOBI (.mobi)" },
    { id: "pdf", label: "PDF (.pdf)" },
    { id: "txt", label: "Text (.txt)" },
    { id: "azw3", label: "AZW3 Kindle (.azw3)" },
  ];
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "E-Books", extensions: ACCEPTED_EXTENSIONS }],
      });
...
<div class="layout">
  <div class="panel main-panel">
    <div class="unified-card">
      <div class="header">
        <h2>E-Book Converter</h2>
        <p>Convert EPUB, PDF, MOBI, AZW3, TXT</p>
      </div>
        ...
        <div class="file-list-scroll">
          {#if selectedFiles.length === 0}
            <div class="empty-state">
              <i class="nf-md-book_open_page_variant"></i>
              <p>Drag e-books here</p>
            </div>
          {:else}
