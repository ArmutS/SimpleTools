<script lang="ts">
  ...
  const ACCEPTED_EXTENSIONS = ["svg", "eps", "ai", "dxf"];
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "Vectors", extensions: ACCEPTED_EXTENSIONS }],
      });
...
<div class="layout">
  <div class="panel main-panel">
    <div class="unified-card">
      <div class="header">
        <h2>Vector Converter</h2>
        <p>Convert SVG, EPS, AI</p>
      </div>
        ...
        <div class="file-list-scroll">
          {#if selectedFiles.length === 0}
            <div class="empty-state">
              <i class="nf-md-vector_curve"></i>
              <p>Drag vectors here</p>
            </div>
          {:else}
