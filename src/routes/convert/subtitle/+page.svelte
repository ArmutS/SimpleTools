<script lang="ts">
  ...
  const ACCEPTED_EXTENSIONS = ["srt", "vtt", "ass", "ssa", "sub", "sbv"];
  const TARGET_FORMATS = [
    { id: "srt", label: "SRT Subtitle (.srt)" },
    { id: "vtt", label: "WebVTT (.vtt)" },
    { id: "ass", label: "ASS/SSA (.ass)" },
  ];
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "Subtitles", extensions: ACCEPTED_EXTENSIONS }],
      });
        ...
        <div class="file-list-scroll">
          {#if selectedFiles.length === 0}
            <div class="empty-state">
              <i class="nf-md-closed_caption"></i>
              <p>Drag subtitles here</p>
            </div>
          {:else}
