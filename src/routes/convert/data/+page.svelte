<script lang="ts">
  ...
  const ACCEPTED_EXTENSIONS = ["json", "xml", "csv", "yaml", "yml", "toml"];
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "Data Files", extensions: ACCEPTED_EXTENSIONS }],
      });
  ...
  <div class="panel main-panel">
    <div class="unified-card">
      <div class="header">
        ...
        <p>Convert JSON, XML, CSV, YAML, TOML</p>
      </div>
        ...
        <div class="file-list-scroll">
          {#if selectedFiles.length === 0}
            <div class="empty-state">
              <i class="nf-md-database"></i>
              <p>Drag data files here</p>
            </div>
          {:else}
