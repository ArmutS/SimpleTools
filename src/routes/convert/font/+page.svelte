<script lang="ts">
  ...
  const ACCEPTED_EXTENSIONS = ["ttf", "otf", "woff", "woff2", "eot", "fnt"];
  const TARGET_FORMATS = [
    { id: "woff2", label: "Web Font (.woff2)" },
    { id: "woff", label: "Web Font (.woff)" },
  ...
  async function selectFiles() {
    try {
      ...
      const selected = await open({
        multiple: true,
        filters: [{ name: "Fonts", extensions: ACCEPTED_EXTENSIONS }],
      });
  ...
  <div class="panel main-panel">
    <div class="unified-card">
      <div class="header">
        ...
        <p>Convert TTF, OTF, WOFF, WOFF2</p>
      </div>
